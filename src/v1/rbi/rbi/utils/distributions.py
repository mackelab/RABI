...
class SIRDistirbution(torch.distributions.Distribution):
    arg_constraints ={}
    def __init__(self, proposal: torch.distributions.Distribution, potential_fn: Callable, context: torch.Tensor, K: int=4, mc_samples=1000, validate_args=None):
        ...
        #Evidence approx
        proposed_samples = self.proposal.sample((self.mc_samples, self.K - 1))
        logweights_samples = self.potential_fn(self.context, proposed_samples).squeeze() - self.proposal.log_prob(proposed_samples).squeeze()
        self.evidence_approx = torch.logsumexp(logweights_samples - math.log(self.K), 1)
    
        
        super().__init__(self.proposal.batch_shape, self.proposal.event_shape, validate_args)
    ...
    def sample(self, shape=()):
        ...
        log_potential = torch.nan_to_num(log_potential, nan=-10000, posinf=10000, neginf=-10000)
        ...
        cum_importance_weights_normalized = torch.softmax(log_importance_weights, 0).cumsum(0)
        u = torch.rand(cum_importance_weights_normalized.shape[1:])
        while u.ndim < cum_importance_weights_normalized.ndim:
            u = u.unsqueeze(0)

        mask = torch.cumsum(cum_importance_weights_normalized >= u, 0) == 1.
        samples = proposed_samples.squeeze().T[:, mask.T].reshape(self.event_shape + self.batch_shape + shape).T
    ...
    def log_prob(self, val):
        ...
        logprob = torch.logsumexp( log_potential.unsqueeze(0) - total_logweights - math.log(self.mc_samples), 0)
        return logprob
...
class MCMCDistirbution(torch.distributions.Distribution):
    arg_constraints ={}
    def __init__(self, proposal: torch.distributions.Distribution, potential_fn: Callable, context: torch.Tensor, kernel_std=0.1, warmup_steps=500, num_chains=100, thinning=3, mc_samples=100, validate_args=None):
        self.proposal = proposal 
        self.potential_fn = potential_fn
        self.context = context
        self.mc_samples = mc_samples
        ...
        logweights_samples = self.potential_fn(self.context, proposed_samples).squeeze() - self.proposal.log_prob(proposed_samples).squeeze()
        self.evidence_approx = torch.logsumexp(logweights_samples - math.log(self.mc_samples), 0)
        super().__init__(self.proposal.batch_shape, self.proposal.event_shape, validate_args)
...
class EmpiricalDistribution(MixtureSameFamily):
    ...
    def __init__(self, samples,event_dim=1, samples_dim = -2,validate_args=False):
        component_distribution = torch.distributions.Independent(DeltaDistribution(samples), event_dim)
        mixture_distribution = torch.distributions.Categorical(logits = torch.zeros(samples.shape[:samples_dim+1]))
...
class _MixDiagNormalSample(Function):
    ...
    def backward(ctx, grad_output):
        ...
        locs_grad[~torch.isfinite(locs_grad)] = 0.
        scales_grad[~torch.isfinite(scales_grad)] = 0.
        logits_grad[~torch.isfinite(logits_grad)] = 0.
