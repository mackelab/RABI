...
from typing import Any, Optional, Union, List, Callable, Dict, Type
    ...
    **kwargs,
    ...
    for metric_class, metric_params in zip(metric_classes, metric_params):  # type: ignore
        ...
        if "kernel" in metric_params:
            k = get_kernel(**metric_params["kernel"], model=model, xs=torch.concat([xs[:500], xs[:500] + (torch.rand_like(xs[:500])-0.5)]))
...
def run_approx_eval_tilde(
    ...
    **kwargs,
    ...
    for metric_class, metric_params in zip(metric_classes, metric_params):  # type: ignore
        ...
        if "kernel" in metric_params:
            k = get_kernel(**metric_params["kernel"], model=model, xs=torch.concat([x_unsecure, x_adversarial], dim=0))
            metric_params["kernel"] = k
