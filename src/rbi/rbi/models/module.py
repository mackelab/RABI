...
class ExchangableLinearLayer(nn.Module):
    ...
    def forward(self, x) -> Tensor:
        ...
        h = torch.nan_to_num(x)
        h = self.phi(x)
        h = h * mask.float()

        if self.aggregation_fn is torch.sum:
            out = self.aggregation_fn(h, dim=self.aggregation_dim)  # type: ignore
            out = self.g(out)
        elif self.aggregation_fn is torch.mean:
            correction_factor = mask.shape[self.aggregation_dim] * mask.sum(dim=-1)
            out = self.aggregation_fn(h, dim=self.aggregation_dim)
            out = correction_factor * out
        elif self.aggregation_fn is torch.min:
            h[mask] = torch.inf
            out = self.aggregation_fn(h, dim=self.aggregation_dim)
        elif self.aggregation_fn is torch.max:
            h[mask] = -torch.inf
            out = self.aggregation_fn(h, dim=self.aggregation_dim)
        else:
            out = self.aggregation_fn(h, dim=self.aggregation_dim)
