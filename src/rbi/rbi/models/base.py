...
from rbi.utils.distributions import ConditionalTransformedDistribution, MixtureDistribution
...
class ParametricProbabilisticModel(nn.Module):
    ...
    def output_transform(self, transform):
        ...
        if transform is not None:
            self.generator = GeneratorWithTransform(self.generator, transform)
        else:
            if isinstance(self.generator, GeneratorWithTransform):
...
class PyroFlowModel(nn.Module):
    ...
    def output_transform(self, transform):
        ...
        if transform is not None:
            if len(self.transforms) == self._transforms:
                self.transforms.append(transform)
            else:
...
class EnsembleModel(nn.Module):
    def __init__(self, models):
        ...
        self.mixture_distribution = torch.distributions.Categorical(logits = torch.zeros(len(models)))
        self.embedding_net = nn.Identity()
    ...
    def forward(self, x):
        ...
        return MixtureDistribution(self.mixture_distribution, qs)

    def to(self, device):
        self.embedding_net.to(device)
        self.mixture_distribution = torch.distributions.Categorical(logits = torch.zeros(len(self.models), device=device))
