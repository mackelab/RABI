...
from rbibm.runs.run_true_posterior_samples import run_true_posterior_samples
...
def rbibm(cfg: DictConfig) -> Optional[float]:
    ...
    sweeper = cfg.sweeper   # Required as local variable
    ...
    try:
        ...
        log.info(e)
        # If some cuda problem then switch to cpu...
    ...
    if cfg.run_eval:
        ...
        if cfg.run_eval_rob:
            ...
            for eps in tolerance_levels:
                ...
                # Each eps on same seed to make it independent of order
                try:
                    set_seed(seed)
                    (
                        id_adversarial,
                        ...
                        x_adversarial,
                        runtime_rob,
                    ) = eval_robustness_metric(
                        name, model, id, task, test_loader, eps, cfg, log
                    )
                    id_adversarials.append(id_adversarial)
            ...
            if cfg.run_eval_approx:
                ...
                for id_adversarial in id_adversarials:
                    ...
                    # Each evaluation on same seed...
                
                    (
                        x_unsecure,
                        theta_unsecure,
                        x_adversarial,
                    ) = get_adversarial_examples_by_id(name, id_adversarial)
                    log.info("\nCompute approximaiton metrics for adversarial examples")
                    set_seed(seed)
                    (
                        values_adversarial,
    ...
    if cfg.eval_true.name == "mcmc":
        log.info("Collecting true posterior samples from adversarial examples")
        if 'id_adversarials' not in locals():
            df_rob = query_rob_metric(name, id=id, attack=cfg.eval_rob.attack.attack_class, metric_rob=cfg.eval_rob.metric.metric_class)
            id_adversarials = df_rob.id_adversarial.unique().tolist()
            print(id_adversarials)
        ...
        for id_adversarial in id_adversarials:
            ...
            log.info("Sampling from ground truth with MCMC.")
            set_seed(seed)
            samples_clean_top, samples_tilde_top, samples_clean_rand, samples_tilde_rand = run_true_posterior_samples(task, x_unsecure, x_adversarial, num_samples_top = cfg.eval_true.num_samples_top, num_samples_rand = cfg.eval_true.num_samples_rand, top_xs = cfg.eval_true.top_xs, rand_xs = cfg.eval_true.rand_xs, device=cfg.device)
            save_posterior_samples_by_id(name, id_adversarial, samples_clean_top, samples_tilde_top, samples_clean_rand,samples_tilde_rand)
...
def load_or_train_model(
    ...
    if check_id_exists(name, id) or not cfg.run_train:
        ...
        log.info(f"On device {cfg.device}")
        ...
        if cfg.defense.defense is not None:
            defense_class = getattr(defense_module, defense_name)
        else:
            defense_class = None
        # Optimizer
        ...
        # Start training -> If somethin very bad happend restart and reduce learning rate
        not_trained_successfull = True
        i = 0    
        while not_trained_successfull:
            start_time = time.time()
            new_model, defense, train_loss, validation_loss, test_loss = run_train(
                task,
                deepcopy(model),
                loss_class,
                train_loader,
            ...
            # Train loss technically can be nan as then the last best model is recovered -> Maybe still restart?
            if  torch.isnan(test_loss) and i < 3:
                i += 1
                # Reduce learning rate
                hyper_parameters_train["lr"] = 0.1 * hyper_parameters_train["lr"]
                log.info("Training failed catastrophically, reduceing learning rate to " + str( hyper_parameters_train["lr"]))
                del new_model

            else:
                # Retry if not successfull
                not_trained_successfull = False 
                model = new_model
        ...
        if cfg.store_model:
            ...
            # Update used learning rate
            train_parmas = dict(cfg.train)
            train_parmas["lr"] = hyper_parameters_train["lr"]

            # Get loss and model names
    ...
    # Depending on which operation is taken the random number generator may be out of sync -> Reseed!
    set_seed(seed)
...
def eval_robustness_metric(
    ...
    if cfg.store_metrics:
        id_adversarial = add_new_rob_metric_entry(
            ...
            attack_loss_fn=attack_loss_fn.__name__
