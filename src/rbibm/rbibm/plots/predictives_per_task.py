...
def plot_gl_predictives(
    ...
    **kwargs
    ...
    if true_thetas is not None:
        true_predictives = simulator(true_thetas)
        predictives.append(true_predictives)
...
def plot_lv_predictives(
    ...
    for x_obs in xs:
        ...
        predictives.append(x_predictives)
    ...
    for i, x_predictives in enumerate(predictives):

        color1 = colors[i]
        color2 = colors[i] #[max(c - 0.3, 0) for c in to_rgb(colors[i])]

        time_points_obs = xs.shape[-1] // 2
        ...
        if i < len(xs):
            x_obs = xs[i].reshape(2, time_points_obs)
...
def plot_sir_predictives(
    ...
    if all_states:
        ...
        for i, predictives in enumerate(predictives):
            S, I, R = predictives
        ...
        axes = plt.gca()
        for i, predictive in enumerate(predictives):
            S, I, R = predictive
...
def plot_hh_predictives(
    ...
    for i, x_predictives in enumerate(predictives):
        ...
        if i < len(xs):
            ax.plot(t_obs, xs[i].squeeze(), lw=lw, color=color)
...
def plot_vae_predictives(
    ...
    points = [x for x in xs]
    if true_thetas is not None:
        true_predictives = simulator(true_thetas[:n_samples])
        predictives += [true_predictives]
        points += [points[-1]]
...
def plot_rps_predictives(
    model, task, xs, ax=None, figsize=None, n_samples=16, true_thetas=None, device="cpu"
):
...
def plot_spatial_sir_predictives(
    model, task, xs, ax=None, figsize=None, n_samples=16, true_thetas=None, nrow=8, grid_padding=5, grid_pad_value=0., grid_spec_width_ratios= (0.95,4), pad_label=0,device="cpu",titles=[
        ["Observation", "Posterior predictive"],
        ["Adversarial example", "Adversarial posterior predictive"],
    ]
):
...
def plot_pyloric_predictives(
    model, task, xs, ax=None, figsize=None, n_samples=16, device="cpu", colors=None, true_thetas=None, titles=["Observation", "Adversarial"]
):
    xs = xs.reshape(-1, xs.shape[-1])
    model = model.to(device)
    simulator = task.get_simulator(device=device)
